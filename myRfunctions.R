...
# ------------------------------------------------------------
clear = function(clearConsole = TRUE,
                 clearFigs = TRUE) {
...
df2list = function(df, col_idx, append_list = NULL) {
  if (is.null(append_list))
    append_list = list()  # Initiate list if needed
  for (i in 1:nrow(df)) {
...
submit_cluster = function(bash_file, n_jobs, ...) {
  ...
  sys_command = paste("sbatch", slurm_array, bash_file, log_file, ...)
...
sumMat = function(x, sumDim) {
  ...
  if (nDims == 2) {
    # 2D matrices included for readability
    if (sumDim == 1) {
  ...
  if (nDims > 2) {
    # Higher dimensional arrays are summed and squeezed
...
adjust_EIR <- function(eir, access) {
  ...
  # Load the model
  trained_model <- readRDS(paste0(file.path("/scicore/home/penny/masthi00", "smc_resistance", "EIR_2_seasonality.RData")))
...
adjust_EIR_2 <- function(eir, access) {
  ...
  # Load the model
  trained_model <- readRDS(paste0(file.path("/scicore/home/penny/masthi00", "smc_resistance", "EIR_3_seasonality.RData")))
